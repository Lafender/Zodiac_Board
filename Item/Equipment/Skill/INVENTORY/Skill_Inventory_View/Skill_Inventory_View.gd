...
export(NodePath) onready var grid = $ScrollContainer/CenterContainer/GridContainer
...
func _build_grid() -> void:
	...
	for item in inventory.item_sums:
		var chip: Skill_Data = inventory.get_item_resource(item)
		var panel: PanelContainer  = preload(skill_chip_panel).instance()
